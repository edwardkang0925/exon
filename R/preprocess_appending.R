...
# Check if the first argument is present, otherwise exit with an error
if (length(args) < 1) {
  stop("No trait argument provided. Please provide the trait as a command-line argument.")
}

# Use the first argument as the trait
# FIXME
#trait <- "fhshdl" #args[1] # ex) fhshdl
...
for (trait in traits){
  ...
  for (bamfile in colnames(rds)){
    # HARDCODED based on the filename format of diffUTR runs on 2561 bamfiles
