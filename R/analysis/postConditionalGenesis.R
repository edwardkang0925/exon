...
for(trait in traits){
  ...
  conditional.genesis.df$nlog10p <- -log10(conditional.genesis.df$p_lrt)
  nlog10p.max <- max(conditional.genesis.df$nlog10p)
  
  # using two thresholds to separate super sig bins and 
  df.nominal.sig <- conditional.genesis.df[conditional.genesis.df$p_lrt < 0.01 / dim(conditional.genesis.df)[1],]
  ...
  nlogp.hist.plot <- ggplot(conditional.genesis.df, aes(x = nlog10p)) +
    ...
    theme_minimal() +
    labs(x = "-log10(p_lrt)", y = "Count",
         title = paste0("Conditional Genesis all: (", trait, ") -log10P ")) +
    theme(plot.title = element_text(size = 28), 
          axis.title = element_text(size = 24), 
          axis.text = element_text(size = 20))
  
  ggsave(paste0("outputs/plots/conditionalAll/hist_nlogp_conditionalGenesis_",trait,"_lrt_all.png"), plot = nlogp.hist.plot, width = 10, height = 7, dpi = 300)
  ...
  nlogp.sig.hist.plot <- ggplot(df.nominal.sig, aes(x = nlog10p)) +
    ...
    annotate("text", x = nlog10p.max, y = Inf, label = paste("Max =", round(nlog10p.max, 2)), vjust = 2, hjust = 1.5, color = "red") +
    theme_minimal() +
    labs(x = "-log10(p_lrt), zoomed", y = "Count",
         title = paste0("Conditional Genesis all: (", trait, ") -log10P")) +
    theme(plot.title = element_text(size = 28), 
          axis.title = element_text(size = 24), 
          axis.text = element_text(size = 20))
  
  ggsave(paste0("outputs/plots/conditionalAll/hist_nlogp_conditionalGenesis_",trait,"_lrt_sig_all.png"), plot = nlogp.sig.hist.plot, width = 10, height = 7, dpi = 300)
