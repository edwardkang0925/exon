...
FDR = 0.01
logPath <- "outputs/compareGeneBin_quickdirty.txt"


#traits <- c("lnTG", "adjTC", "BMI", "fhshdl", "fvc") # traits with sig genes
traits <- c("ABI", "adjLDLF", "fev1fvc", "fev1", "pulse", "waist")
for(trait in traits){
  ...
  sosoExon.df <- exon.pvals.df[exon.pvals.df$pval_bacon < 0.01,]
  
  write(paste(trait, "has", length(sigGenes), "significant genes"), logPath, append = TRUE)
  write(paste(trait, "has", dim(sigExon.df)[1], "significant (0.01/#bins) bins"), logPath, append = TRUE)
